...
\question{Avec le modèle 2 déterminer %la FTBO $F(p)=\dfrac{\text{Mes}\varphi(p)}{\varepsilon_2(p)}$ de ce schéma puis calculer 
la fonction de transfert liant la perturbation et la sortie $\text{Pert}(p)=\dfrac{\varphi(p)}{\text{Pe}(p)}$.}
\ifprof
\begin{corrige}
...
On simplifie l'expression de la FTBO non corrigée (sans $K_P$) et on note  $F(\omega)= \dfrac{2}{j\omega \left(1+0,4 j\omega \right)}$. Les valeurs prises par le gain et la phase sont donnés dans le tableau suivant.
...
\question{On désire une marge de gain de $M_G \geq \SI{5}{dB}$ et une marge de phase $M\varphi \geq 20\degres$ (exigence 1.1.3 « Stabilité de la commande »). Déterminer la valeur maximale de $K_P$ en utilisant les données dans le tableau.}
% (OU 40 ?degres)
...
\ifprof
\else
Le figure \ref{fig_cy_02_ch_01_td_02_fig7} (droite) présente la réponse temporelle de l’axe de tangage à une perturbation sinusoïdale (due par
...
\caption{Réponse de l'axe à une perturbation sinusoïdale \label{fig_cy_02_ch_01_td_02_fig7}}
\end{figure*}
