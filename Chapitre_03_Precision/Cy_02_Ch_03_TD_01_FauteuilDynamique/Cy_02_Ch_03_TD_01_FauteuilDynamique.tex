% "{'classe':('PSI'),'chapitre':'slci_precision','type':('td'),'titre':'Fauteuil dynamique de cinéma', 'source':'Centrale Supelec 2015 - TSI','comp':[],'corrige':True}"
%\setchapterimage{fig_01.png}
\chapter*{TD \arabic{cptTD} \\ 
Fauteuil dynamique de cinéma -- \ifprof Corrigé \else Sujet \fi}
\addcontentsline{toc}{section}{TD \arabic{cptTD} : Fauteuil dynamique de cinéma -- \ifprof Corrigé \else Sujet \fi}
...
\marginnote{Concours Centrale-Supélec TSI 2015}
\marginnote{\xpComp{PERF}{05}\xpComp{PERF}{06}}

\begin{marginfigure}
\includegraphics[width=.7\linewidth]{image1}
\end{marginfigure}
...
%%
%%
%%\question{À l'aide d'une fermeture géométrique, exprimer littéralement l'angle
...
%%\end{figure}
%
%\question{Déterminer par linéarisation autour du point de fonctionnement
...
%
%\question{En tenant compte des informations précédentes, calculer la valeur
...
%
%
%\question{Pour chacun des deux modèles (initial et simplifié), quelle est la
...
\caption{Modèle de la boucle d'asservissement de vitesse \label{fig13}}
\end{figure}
%\end{figure}
...
$J=\SI{0,16e-3}{kg.m^{2}}$;
$K_{\Omega} = \SI{4.1e-4}{N.m.s.rad^{-1}}$;.
...
\question{Exprimer la fonction de transfert de la boucle de vitesse
...
\question{$T_1$ étant égal à $J/f$, montrer alors que
...
\question{En déduire, à l'aide de la figure précédente, $\theta(p)/C_R(p)$
...
\begin{figure}[!h]
\centering
\includegraphics[width=.7\linewidth]{image15.png}

\caption{Modèle simplifié de la boucle d'asservissement de position \label{fig14}}
\end{figure}

\fi


\question{Exprimer la fonction de transfert $\theta(p)/\theta_C(p)$.
...
%\question{Tracer le diagramme de Bode de la fonction de le fonction de transfert en boucle ouverte. Quel que soit le résultat de la partie précédente, on prendra $a=\SI{0,1}{V}$.}
...
\question{ Exprimer dans un premier temps $\mu(p)$ en fonction de
...
%
%\question{Déterminer, à partir du paramétrage donné figure \ref{fig8}, l'expression
...
%  
%\question{Conclure quant au respect du nombre de g du cahier des charges (tableau \ref{tab:4}) vis-à-vis des accélérations simulées produites par le dosseret du
...
\caption{Structure avec anticipation \label{fig16}}
\end{figure}
\fi

\question{\label{q13}Déterminer l'erreur de position $\mu$\emph{\textsubscript{p}} puis
...
\question{\label{q14}D'après l'erreur de traînage $\mu$\emph{\textsubscript{v}} déterminée à
...
\caption{Accélération du dosseret avec et sans anticipation \label{fig17}}
\end{figure}
%\end{figure}
\fi
...
\caption{Commande simultanée des deux moteurs \label{fig19}}
\end{figure}

\fi

\question{En réutilisant éventuellement les calculs effectués aux questions \ref{q13} et \ref{q14} et en
...
\question{Conclure quant à l'erreur en accélération lors de la commande
  simultanée.}
\ifprof
\begin{corrige}
\end{corrige}
...
\caption{$\theta_1-\theta_2$ en fonction du temps \label{fig20}}
\end{marginfigure}
%\end{figure}
\fi
...
\begin{marginfigure}
\centering
\includegraphics[width=3cm]{Cy_02_Ch_03_TD_01_qr}
\end{marginfigure}
\fi
