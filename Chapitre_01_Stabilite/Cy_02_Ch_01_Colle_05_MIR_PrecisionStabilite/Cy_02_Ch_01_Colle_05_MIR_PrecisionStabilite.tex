% "{'classe':('PSI'),'chapitre':'slci_stabilite','type':('colle'),'titre':'Robot MIR : Machine d\\'inspection des réacteurs rapides', 'source':'E3A MP - 2012 ','comp':[],'corrige':False}"
...
% $$
%avec $u(t)$ échelon unitaire\sidenote{En passant dans le domaine de Laplace, $u(t)$ est donc transformé en $\dfrac{1}{p}$}.
%
%
...
%%$f_{\text{pert}}(t) = -m_t  \sin \alpha u(t)$ avec $u(t)$ F unitaire.
%
%
%\question{En supposant des conditions initiales nulles, et que $\indice{F}{pert}(p)=0$ exprimer les fonctions de transfert $A(p)$ et $B(p)$ en fonction entre autres de $\delta$, $\beta$ et $M_{eq}$.
%}
%\ifprof
%\begin{corrige}
%\end{corrige}
...
\centering
\includegraphics[width=3cm]{Cy_02_Ch_01_Colle_05_MIR_PrecisionStabilite_qr}
\end{marginfigure}
\fi
