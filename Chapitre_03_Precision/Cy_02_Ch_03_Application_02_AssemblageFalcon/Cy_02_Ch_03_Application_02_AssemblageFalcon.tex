% "{'classe':('PSI'),'chapitre':'slci_precision','type':('application'),'titre':'Cellule d\\'assemblage pour avion Falcon', 'source':'E3A - PSI 2015','comp':[],'corrige':True}"
%\setchapterimage{fig_00_bis}
\chapter*{Application \arabic{cptApplication} \\ 
Cellule d'assemblage pour avion Falcon  \ifprof Corrigé \else Sujet \fi}

\addcontentsline{toc}{section}{Application \arabic{cptApplication} :  Cellule d'assemblage pour avion Falcon -- \ifprof Corrigé \else Sujet \fi}
...
\marginnote{$H_M (p)=\dfrac{K_M}{(1+T_E p)(1+T_M p)}$ et
...
\question{ Calculer l'écart statique et l'écart de trainage pour $C(p)=K_p$. Pouvait-on prévoir le résultat ?}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
\question{Calculer l'écart statique et l'écart de trainage pour $C(p)=\dfrac{K_i}{p}$. Pouvait-on prévoir le résultat ?}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
\begin{marginfigure}
\centering
\includegraphics[width=3cm]{Cy_02_Ch_03_Application_02_qr}
\end{marginfigure}
\fi
